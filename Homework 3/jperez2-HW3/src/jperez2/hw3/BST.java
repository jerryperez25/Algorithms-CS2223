...
public class BST<Key extends Comparable<Key>, Value extends Comparable<Value>> {
    ...
    {
    	return leafFinder(root); 
    }
    
    public int leafFinder(Node thisone)
    {
    	if(thisone == null)
    	{
    		return 0;
    	}
    	if(thisone.left == null && thisone.right == null)
    	{
    		return 1;
    	}
    	else
    	{
    		return (leafFinder(thisone.left)+leafFinder(thisone.right));
    	}
    }
    ...
    { 
    	best = root;
    	return maxValueHelp(root);
    }
    
    public Key maxValueHelp(Node thisone)
    {
    	Node curr = thisone;
    	if(best != null) 
    	{
    		if(curr.val.compareTo(best.val)>0)
    		{
    			best = thisone;
    		}
    	}
    	...
    	if(curr.left != null)
    	{
    		maxValueHelp(thisone.left);
    	}
    	if(curr.right != null)
    	{
    		maxValueHelp(thisone.right);
    	}
    	return best.key;
    }
